 ...
 * 
 * @param element
 *            The HTML select element to be sorted
 * @param value
 *            The option value to search for
 */
 ...
 * @param element
 *            The HTML select element to remove the options from
 * @param label
 *            The label text to appear in the menu option
 * @param value
 *            The value of the option element to add
 * @param before
 *            (Optional) The option to add this option before (either a numeric
 *            index or a value)
 */
FM.addOption = function(element, label, value, before) {
    if (label == undefined || label == '' || value == undefined || value == '') {
        ...
        // First, start an array with the new option and copy all options from
        // the
        ...
        for ( var i = 0, j = before; i < options.length; i++, j++) {
            element.options[j] = new Option(options[i].text, options[i].value, options[i].defaultSelected, options[i].selected);
        }
    }
} // addOption()
 ...
 *            The HTML select element to remove the options from
 * @param action
 *            The Action to perform. One of: remove, select, deselect
 * @param options
 *            (Multiple) Option indexes or values to perform the action on
 */
FM.alterOptions = function(element, action) {
    ...
    if (action == undefined || action != 'remove' && action != 'select' && action != 'deselect') {
        ...
        // Step through the options backwards and alter them if they are in the
        // list
 ...
 * @author Dan Delaney http://fluidmind.org/
 * @param element
 *            The HTML select element with the options to me moved
 * @param index1
 *            First index number of the options array to swap
 * @param index2
 *            Second index number of the options array to swap
 */
FM.swapOptions = function(element, index1, index2) {
    ...
    if (index1 != index2 && index1 >= 0 && index1 < element.options.length && index2 >= 0 && index2 < element.options.length) {
        ...
        element.options[index1] = new Option(element.options[index2].text, element.options[index2].value, element.options[index2].defaultSelected,
        ...
        // Then select the ones we swapped, if they were selected before the
        // swap
...
/**
 * Shifts an option (specified by index or value) in a select element up one
 * position
 * 
 * @author Dan Delaney http://fluidmind.org/
 * @param element
 *            The form select element with the options to me shifted
 * @param option
 *            The index or value of the option to be shifted up
 * @see swapOptions
 */
FM.shiftOptionUp = function(element, option) {
    ...
    if (isNaN(option))
        option = FM.findOption(element, option);
...
/**
 * Shifts an option (specified by index or value) in a select element down one
 * position
 * 
 * @author Dan Delaney http://fluidmind.org/
 * @param element
 ...
 *            The HTML select element to be sorted
 * @param direction
 *            The sort direction: 'asc' or 'desc' (optional)
 */
FM.sortOptions = function(element, direction) {
    ...
    // Sort it with a function that uses the 'text' property of the Option
    // object
    ...
    for ( var i = 0; i < options.length; i++) {
        element.options[i] = new Option(options[i].text, options[i].value, options[i].defaultSelected, options[i].selected);
    }
} // sortOptions()
 ...
 * @author Dan Delaney http://fluidmind.org/
 * @param element
 *            The HTML select element to make the selection in
 */
 ...
 * @author Dan Delaney http://fluidmind.org/
 * @param element
 *            The form select element with the options to me moved
 * @see swapOptions
 */
...
/**
 * Copies selected options in a multiple-select element to another
 * multiple-select element.
 * 
 * @author Dan Delaney http://fluidmind.org/
 * @param fromElement
 *            The HTML select element with the options to be copied
 * @param toElement
 *            The HTML select element to copy the options to
 */
FM.copySelections = function(fromElement, toElement) {
    if (typeof fromElement == 'string')
        fromElement = document.getElementById(fromElement);
    ...
    if (fromElement.type == 'select-multiple' && toElement.type == 'select-multiple') {
        ...
        for ( var i = 0; i < fromElement.options.length; i++) {
            ...
            if (fromElement.options[i].selected) {
                ...
                toElement.options[toElement.options.length] = new Option(fromElement.options[i].text, fromElement.options[i].value,
                        fromElement.options[i].defaultSelected, fromElement.options[i].selected);
            }
        } // for
    }
} // copySelections()
...
/**
 * Moves selected options in a multiple-select element to another
 * multiple-select element.
 * 
 * @author Dan Delaney http://fluidmind.org/
 * @param fromElement
 *            The HTML select element with the options to be moved
 * @param toElement
 *            The HTML select element to move the options to
 * @see copySelections
...
FM.shiftSelectionsTop = function(element) {
    if (typeof element == 'string') {
        element = document.getElementById(element);
    }
    try {
        ...
        if (element.type == 'select-multiple') {
            ...
            for ( var i = 0; i < element.options.length; i++) {
                if (element.options[i].selected) {
                    selecteds.push(element.options[i]);
                } else {
                    noSelecteds.push(element.options[i]);
                }
            }
        } 
        ...
        for (var i = 0; i < selecteds.length ; i++) {
            element.options.add(selecteds[i]);
        }

        for (var i = 0; i < noSelecteds.length ; i++) {
            element.options.add(noSelecteds[i]);
        }
...
FM.shiftSelectionsBottom = function(element) {
    if (typeof element == 'string') {
        element = document.getElementById(element);
    }
    try {
