...
function DetalleDlgEditar(record){
    ...
    try {
        ...
        for (var i = 0; i < comboCanal.length; i++) {
            if (canalEnvioVar == comboCanal[i].value) {
            	Y.publishMsg("div.canalEnvio", comboCanal[i].label);
            }
        }
        
        generales.elements['notificacion.canalEnvio'].style.display = 'none';
        ...
        generales.elements['notificacion.canalEnvio'].value = record.getData('canalEnvio');
        ...
        generales.elements['notificacion.canalEnvio'].disabled = true;
